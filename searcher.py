...
class Searcher:
    ...
    def word2vec_expansion(self, query_as_list, inv_index):
        ...
        for term in query_as_list:
            if term.lower() in self.model:
                most_similar = self.model.wv.most_similar(term.lower(), topn=1)
    ...
    def word2vec_replacement(self, query_as_list, inv_index):
        ...
        for term in query_as_list:
            if term.lower() in self.model:
                most_similar = self.model.wv.most_similar(term.lower(), topn=3)
                for similar in most_similar:
                    if similar.upper() in inv_index:
                        new_query_as_list.append(similar)
                    elif similar.lowwer() in inv_index:
                        new_query_as_list.append(similar)
                    else:
                        continue
        return new_query_as_list
